...

      {
        ...
        const stdoutServer = net.createServer(s => {
          ...
          if (cache.state === 'LIVE') {
            if (cache.k) {
              {
                const d = cache.k.stdout.pipe(s, {end: false});
                s.once('end', () => {
                  d.unpipe(cache.k.stdout);
                  d.unpipe(s);
                  cache.k.stdout.unpipe(s);
                  cache.k.stdout.unpipe(d);
                });
              }
...

      {
        ...
        const stderrServer = net.createServer(s => {
          ...
          if (cache.state === 'LIVE') {
            if (cache.k) {
              {
                const d = cache.k.stderr.pipe(s, {end: false});
                s.once('end', () => {
                  cache.k.stderr.unpipe(s);
                  cache.k.stderr.unpipe(d);
                  d.unpipe(cache.k.stderr);
                  d.unpipe(s);
                });
              }
            }
